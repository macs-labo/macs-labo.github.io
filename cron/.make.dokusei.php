...
create table if not exists spec.info (item varchar primary key, value varchar);
insert or replace into spec.info (item, value) values ('suisan', '$date');
drop table if exists spec.suisan;
create table spec.suisan (bango integer, chuijiko varchar);
begin transaction;\n
SUISAN;
...
if ($excel) {
  ...
  foreach ($rows as $r) {
    ...
    $sql .= "insert into spec.suisan values ($out);\n";
  }
  ...
  unlink($xfile);
  $total += microtime(true);
  logputs($suisan, "Created $total");
  if ($debug) echo "$suisan Created $total\n";
  $fupdated = true;
}
...
/* 製剤追加情報テーブル $date */
create table if not exists spec.info (item varchar primary key, value varchar);
insert or replace into spec.info (item, value) values ('seizai', '$date');
drop table if exists spec.seizai;
create table spec.seizai (bango integer primary key, dokusei varchar, kousin varchar);
begin transaction;\n
SEIZAI;
$excel = read_excel($xfile);
if ($excel) {
  ...
  foreach ($rows as $r) {
    ...
    if (!preg_match('/^[0-9]+,/', $out)) continue;
    $out = mb_convert_kana($out, 'asKV');
    $sql .= "insert into spec.seizai values ($out);\n";
  }
  ...
  unlink($xfile);
  $total += microtime(true);
  logputs($seizai, "Created $total");
  if ($debug) echo "$seizai Created $total\n";
  $fupdated = true;
}
...
/* 有効成分毒性マスター $date */
create table if not exists spec.info (item varchar primary key, value varchar);
insert or replace into spec.info (item, value) values ('m_dokusei', '$date');
drop table if exists spec.m_dokusei;
create table spec.m_dokusei (ippanmei varchar, nai varchar, yoto varchar, biko varchar, ojas varchar, rac varchar, dokusei varchar, adi varchar, arfd varchar, wqs varchar, eqs varchar, hatsutoroku varchar, jogai varchar);
begin transaction;\n
DOKUSEI1;
$sqlfoot = <<<DOKUSEI2
commit;
drop view if exists spec.dokusei;
create view spec.dokusei as select ippanmei,seibunmei,yoto,dokusei,jogai, biko, ojas, rac, null as seibunEikyo from m_dokusei left join betsumei on dokuseimei = ippanmei;\n
DOKUSEI2;
$excel = read_excel($xfile);
if ($excel) {
  $rows = $excel->rows(0);
  foreach ($rows as $r) {
    ...
    $out = str_replace("\t", ',', $out);
    $sql .= "insert into spec.m_dokusei values ($out);\n";
  }
  $sql .= $sqlfoot;
  ...
  unlink($xfile);
  $total += microtime(true);
  logputs($dokusei, "Created $total");
  if ($debug) echo "$dokusei Created $total\n";
  $fupdated = true;
}

if ($fupdated) {
  require_once '.make.spec.php';
}
